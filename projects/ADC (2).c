...
/* Private macro -------------------------------------------------------------*/
/* USER CODE BEGIN PM */
uint8_t Tim_Flag = 1,Duty = 0;
uint16_t ADC0_Value,ADC0_Volt;
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if(Tim_Flag==1)
    {
      ...
      Duty= ADC0_Value*100 /4096;
      __HAL_TIM_SetCompare(&htim2,TIM_CHANNEL_1,Duty);      
      uint8_t str[64];
      sprintf((char *)str,"采样值：%d，电压：%d.%d%dV\r\n",ADC0_Value,ADC0_Volt/100,(ADC0_Volt%100)/10,ADC0_Volt%10);
      HAL_UART_Transmit(&huart2,(uint8_t*)str,40,10000);
    }
