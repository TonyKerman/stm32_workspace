...
void steeringWheel_executor(steering_wheel_t *this)
{
    ...
    if(this->state==RUNNING)
    {
       //_Swheel_rMotor_posServo(this, this->target_direction +
        ...
        _Swheel_rMotor_posServo(this, (this->target_direction-this->direction)+this->rotation_pos);
...
void _get_min_dirction_err(const float target_direction,const float direction,float*min_err,int8_t*invert)
{

    *min_err=target_direction-direction;
    float abs_err= fabsf(target_direction-direction);
    if (abs_err<=M_PI_2)
    {
        *invert = 1;
    } else
