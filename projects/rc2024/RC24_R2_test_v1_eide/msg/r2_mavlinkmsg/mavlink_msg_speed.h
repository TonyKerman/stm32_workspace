...
#define MAVLINK_MSG_ID_SPEED_CRC 51
#define MAVLINK_MSG_ID_1_CRC 51
...
#define MAVLINK_MESSAGE_INFO_SPEED { \
    ...
    {  { "vx", NULL, MAVLINK_TYPE_DOUBLE, 0, 0, offsetof(mavlink_speed_t, vx) }, \
         ...
         { "vw", NULL, MAVLINK_TYPE_DOUBLE, 0, 16, offsetof(mavlink_speed_t, vw) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_SPEED { \
...
{
    return mavlink_msg_speed_pack(system_id, component_id, msg, speed->vx, speed->vy, speed->vw);
}
...
{
    return mavlink_msg_speed_pack_chan(system_id, component_id, chan, msg, speed->vx, speed->vy, speed->vw);
}
...
static inline void mavlink_msg_speed_send(mavlink_channel_t chan, double vx, double vy, double vw)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_SPEED_LEN];
    _mav_put_double(buf, 0, vx);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_speed_send(chan, speed->vx, speed->vy, speed->vw);
#else
 ...
 */
static inline void mavlink_msg_speed_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  double vx, double vy, double vw)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
...
static inline void mavlink_msg_speed_decode(const mavlink_message_t* msg, mavlink_speed_t* speed)
{
    ...
    speed->vw = mavlink_msg_speed_get_vw(msg);
#else
