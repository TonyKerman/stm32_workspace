...
void swChassis_executor(swChassis_t *this)
{
    ...
    for(int i=0;i<this->swheel_num;i++)
    {
        ...
        if(hDJI[i].speedPID.output>200)
            dji_output_withfc[i]=hDJI[i].speedPID.output+hDJI[i].f_current;
        else if(hDJI[i].speedPID.output<-200)
            dji_output_withfc[i]=hDJI[i].speedPID.output-hDJI[i].f_current;
        else
...
void calculate_target_velocity(swChassis_t *this)
{
    ...
    if(vw!=0)
    {
        ...
        for (int i = 0; i < this->swheel_num; i++) {
            this->wheels[i].target_direction = atan2(Iy-swChassis_wheel_y[i],Ix-swChassis_wheel_x[i])-M_PI_2;
            //如果下一个周期可以
            this->wheels[i].main_speed = vw * norm(Ix-swChassis_wheel_x[i],Iy-swChassis_wheel_y[i]);
        }
    } else
    {
        for (int i = 0; i < this->swheel_num; i++) {
