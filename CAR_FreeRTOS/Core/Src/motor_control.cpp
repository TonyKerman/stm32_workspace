...
Motor::Motor(TIM_HandleTypeDef *htim, uint32_t Channel, GPIO_TypeDef *in1_GPIOx, uint16_t in1p, GPIO_TypeDef *in2_GPIOx,
             uint16_t in2p, GPIO_TypeDef *Sa_GPIOx, uint16_t Sap,GPIO_TypeDef *Sb_GPIOx, uint16_t Sbp)
{
...
void Motor::start() {
    ...
    pid_speed.set_args(incremental, 1, 0.1, 0, 1000, 1000);
...
void Motor::TIM_PeriodElapsedCallback() {
    ...
    speed = ct*2*50*40/32/5.2;//tim1周期20ms speed单位：一秒转多少圈*40(满速约1000)
    ct = 0;
    run();

}
/*
 * GPIO_PinState sa =HAL_GPIO_ReadPin(Sb_GPIO_Port,Sb_Pin);;
        GPIO_PinState sb = HAL_GPIO_ReadPin(Sb_GPIO_Port,Sb_Pin);
        if(sa ==GPIO_PIN_RESET)
        {
            if(sb == GPIO_PIN_SET)
            {
                ct++;
