...
void ps2_receive(SPI_HandleTypeDef * spihandle,ps2outputTypeDef & data)
{
    ...
    HAL_SPI_TransmitReceive(spihandle,&cmd[0],&rawdata[0],1,100);
    HAL_Delay(1);
    HAL_SPI_TransmitReceive(spihandle,&cmd[1],&rawdata[1],1,100);
    HAL_Delay(1);
    HAL_SPI_TransmitReceive(spihandle,&cmd[2],&rawdata[2],1,100);
    for(int i =0;i<9;i++){
        dbg_add(rawdata[i],"val");
    }

    if(rawdata[1]==0x73&&rawdata[2]==0x5A)
    {
