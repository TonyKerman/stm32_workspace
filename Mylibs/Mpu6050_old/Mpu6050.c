...
HAL_StatusTypeDef Mpu6050_getGyroscope(Mpu6050 * me)
{
    ...
    int16_t rawData[3];
    //比率
    float k[4]={2.2875,1.1438,0.5719,0.2859};
...
void Mpup6050_update(Mpu6050 *me)
{
    ...
    // 估计方向的重力
    vx = 2*(me->q1*me->q3 - me->q0*me->q2);
    vy = 2*(me->q0*me->q1 + me->q2*me->q3);
    vz = me->q0*me->q0 - me->q1*me->q1 - me->q2*me->q2 + me->q3*me->q3;
    ...
    // 整合四元数率和正常化
    me->q0 = me->q0 + (-me->q1*gx - me->q2*gy - me->q3*gz)*halfT;
    me->q1 = me->q1 + (me->q0*gx + me->q2*gz - me->q3*gy)*halfT;
    me->q2 = me->q2 + (me->q0*gy - me->q1*gz + me->q3*gx)*halfT;
    me->q3 = me->q3 + (me->q0*gz + me->q1*gy - me->q2*gx)*halfT;

    // 正常化四元
    norm = sqrt(me->q0*me->q0 + me->q1*me->q1 + me->q2*me->q2 + me->q3*me->q3);
    me->q0 = me->q0 / norm;
    me->q1 = me->q1 / norm;
    me->q2 = me->q2 / norm;
    me->q3 = me->q3 / norm;

    Pitch  = asin(-2 * me->q1 * me->q3 + 2 * me->q0* me->q2)* 57.3; // pitch ,转换为度数
    Roll = atan2(2 * me->q2 * me->q3 + 2 * me->q0 * me->q1, -2 * me->q1 * me->q1 - 2 * me->q2* me->q2 + 1)* 57.3; // rollv
    //Yaw = atan2(2*(me->q1*me->q2 + me->q0*me->q3),me->q0*me->q0+me->q1*me->q1-me->q2*me->q2-me->q3*me->q3) * 57.3;                //此处没有价值，注掉
