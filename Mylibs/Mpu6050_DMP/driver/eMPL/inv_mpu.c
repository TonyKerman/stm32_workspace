...
{
    HAL_StatusTypeDef state= HAL_I2C_Mem_Write(&hi2c1,(uint16_t)((slave_addr<<1)|0x00),(uint16_t)reg_addr,I2C_MEMADD_SIZE_8BIT,data,length,1000);
    if(state != HAL_OK)
        return 1;
    return 0;
}
...
{
    HAL_StatusTypeDef state= HAL_I2C_Mem_Read(&hi2c1,(uint16_t)((slave_addr<<1)|0x01),(uint16_t)reg_addr,I2C_MEMADD_SIZE_8BIT,data,length,1000);
    if(state != HAL_OK)
        return 1;
    return 0;
}
